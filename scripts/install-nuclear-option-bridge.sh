...
appid="${NUCLEAR_OPTION_APPID:-2168680}"
steam_root="${STEAM_ROOT:-$HOME/.local/share/Steam}"
game_dir="${NUCLEAR_OPTION_GAME_DIR:-$steam_root/steamapps/common/Nuclear Option}"
pfx="${NUCLEAR_OPTION_PFX:-$steam_root/steamapps/compatdata/$appid/pfx}"
...
if [[ ! -d "$game_dir" ]]; then
  echo "Nuclear Option game directory not found: $game_dir" >&2
  exit 1
fi

if [[ ! -d "$pfx" ]]; then
  echo "Proton prefix not found: $pfx" >&2
  echo "Launch Nuclear Option once through Steam, then rerun this script." >&2
  exit 1
fi
...
echo "Installed NPClient bridge for Nuclear Option."
echo "Game: $game_dir"
echo "Prefix: $pfx"
